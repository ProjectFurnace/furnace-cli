...
module.exports = async (directory) => {
  ...
  const questions = [
    { type: 'input', name: 'template', message: "Template:", default: "starter-template", when: useTemplate },
    ...
    { type: 'confirm', name: 'createHook', message: "Create GitHub Webhook", when: current => isGitHub && current.createRepos && currentConfig.gitToken },
  ...
  if (createRepos) {
    ...
    if (currentRepo) console.log(`repository ${remoteUrl} already exists.`);
      ...
      if (org === gitHubUser.login) {
        await githubUtil.createRepositoryForUser(currentConfig.gitToken, remoteUrl, privateRepo);
      } else {
        await githubUtil.createRepositoryInOrg(currentConfig.gitToken, remoteUrl, privateRepo);
      }
    }
    ...
    if (currentRepo) console.log(`repository ${stateRemoteUrl} already exists.`);
      ...
      if (org === gitHubUser.login) {
        await githubUtil.createRepositoryForUser(currentConfig.gitToken, stateRemoteUrl, privateRepo);
      } else {
        await githubUtil.createRepositoryInOrg(currentConfig.gitToken, stateRemoteUrl, privateRepo);
      }
    }
